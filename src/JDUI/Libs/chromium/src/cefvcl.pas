...
interface
uses
  WinApi.Windows, WinApi.Messages, Classes,
  cefgui, ceflib,
{$ifdef DELPHI16_UP}
...
type
  ...
  public
    ...
    property Tag64: Int64 read FTag64 write FTag64 default 0;
...
begin
  ...
  begin
    FillChar(info, SizeOf(info), 0);
    info.parent_window := FFormHandle;
...
begin
  ...
  try
    try
      ...
      begin
        try
          if FBrowser.IsLoading then FBrowser.StopLoad;
        except
    ...
    try
      if FHandler <> nil then (FHandler as ICefClientHandler).Disconnect;
    except
    end;
