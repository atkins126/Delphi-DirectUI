...
begin
  ...
  FCanvas := dx_vgcore.DefaultCanvasClass.Create(FLayerWidth, FLayerHeight);
  ShowHint := true;
...
begin
  ...
  begin
    if dx_vgcore.aniThread <> nil then
    begin
      dx_vgcore.aniThread.Free;
    end;
    dx_vgcore.aniThread := nil;
  end;
  ...
  begin
    ...
    try
      ...
      begin
        ...
        ScaleMatrix := dx_vgcore.IdentityMatrix;
...
//    FDesignPopup.OwnerDraw := true;
    ...
    begin
      ...
      CatList.Sorted := true;
      for i := 0 to dx_vgcore.ObjectList.Count - 1 do
      begin
        S := TvxObjectClass(dx_vgcore.ObjectList.Objects[i]).ClassName;
        if Pos('Tvx', S) = 1 then
          Delete(S, 1, 3);
        CatList.Add(S);
      end;
      ...
      begin
        ...
        OItem.Tag := $FF;
        for j := 0 to dx_vgcore.ObjectList.Count - 1 do
        begin
          S := TvxObjectClass(dx_vgcore.ObjectList.Objects[j]).ClassName;
          if Pos('Tvx', S) = 1 then
            Delete(S, 1, 3);
          if S = CatList[i] then
          begin
