...
uses
  Generics.Collections, PNGImage2,
  GDIPOBJ, {$IFDEF FPC} LCLIntf, LCLType, Types, Controls, Graphics,{$ELSE}
  WinApi.Windows, WinApi.Messages, VCL.Controls, VCL.Graphics,{$ENDIF}
  Classes, SysUtils, GR32_System, jpegdec, FreeType, jinTextDraw;
...
var
  ...
  GR32_TEXT_NORMAL_FONT: String = '';
  GR32_TEXT_EMOJI_FONT: String = '';
  GR32_TEXT_BOLD_FONT: String = '';
  GR32_TEXT_HEAVY_FONT: String = '';
...
{$ENDIF}
  ...
  public
    ...
    procedure ChangeAlpha(const AlphaValue: Byte);
    procedure PARGB_TO_ARGB;
    procedure ClearColor(const AColor: TColor32);
    procedure GrayScale(const AlphaValue: Byte = 255);
...
{$ENDIF}
    ...
    function TextExtentEx(const Text: String): TSize;
    function  TextExtent(const Text: String): TSize;
    function  TextHeight(const Text: String): Integer;
    function  TextWidth(const Text: String): Integer;
    procedure RenderText_Old(X, Y: Integer; const Text: String; Color: TColor32; AClearType: Boolean);
    procedure RenderTextShadown_old(X, Y: Integer; const Text: String; AALevel: Integer; Color: TColor32; ASize: Integer);
    procedure RenderText(X, Y: Integer; const Text: String; Color: TColor32; AClearType: Boolean);
    procedure RenderTextShadown(X, Y: Integer; const Text: String; Color: TColor32; ASize: Integer; AClearType: Boolean);
...
var
  StockBitmap: TBitmap;
  RenderTextBitmap: TBitmap32;
  RenderTextBitmap2: TBitmap32;
...
procedure TCustomBitmap32.PARGB_TO_ARGB;
var
  X, Y: Integer;
...
begin
  ...
  begin
    P1 := PColor32EntryArray(ScanLine[Y]);
    for X := 0 to Width - 1 do
    begin
      if P1[X].A = 0 then Continue;

      P1[X].R := Round(P1[X].R * 255 / P1[X].A);
      P1[X].G := Round(P1[X].G * 255 / P1[X].A);
      P1[X].B := Round(P1[X].B * 255 / P1[X].A);
    end;
  end;
end;

procedure TCustomBitmap32.ChangeAlpha(const AlphaValue: Byte);
var
  X, Y: Integer;
  P1: PColor32EntryArray;
begin
  ...
  begin
    ...
    for X := 0 to Width - 1 do
    begin
      //P1[X].R := P1[X].R * AlphaValue shr 8;
      //P1[X].G := P1[X].G * AlphaValue shr 8;
      //P1[X].B := P1[X].B * AlphaValue shr 8;
      P1[X].A := P1[X].A * AlphaValue shr 8;
    end;
  end;
end;

procedure TCustomBitmap32.ClearColor(const AColor: TColor32);
var
  R,
  G,
  B,
  ...
  P1: PColor32EntryArray;
begin
  A := AlphaComponent(AColor);
  for Y := 0 to Height - 1 do
  begin
    P1 := PColor32EntryArray(ScanLine[Y]);
    for X := 0 to Width - 1 do
    begin
      if (P1[X].ARGB = AColor) then
      begin
        P1[X].R := 0;
        P1[X].G := 0;
        P1[X].B := 0;
...
procedure TCustomBitmap32.GrayScale(const AlphaValue: Byte = 255);
var
  X, Y: Integer;
  P1: PColor32EntryArray;
  Gray: Byte;
begin
  ...
  begin
    ...
    begin
      ...
      for X := 0 to Width - 1 do
      begin
        Gray := (77 * P1[X].R + 151 * P1[X].G + 28 * P1[X].B) shr 8;
        P1[X].R := Gray;
        P1[X].G := Gray;
        P1[X].B := Gray;
      end;
...
begin
  ...
  begin
    ...
    begin
      B := JpegDecode((Stream as TMemoryStream).Memory, Stream.Size);
      if B <> nil then
      try
        Assign(B);
        Changed;
...
begin
  ...
  {
  if SameText(ExtractFileExt(FileName), '.jpg') or SameText(ExtractFileExt(FileName), '.jpeg') then
  begin
    try
      LoadJPEGFromFile(Self, FileName);
      Exit;
    except
    end;
  end;
  ...
  try
    ...
    AStream.ReadBuffer(Header, SizeOf(TBmpHeader));
    if Header.bfType = $4D42 then
    begin
      // if it is, use our stream read method...
      AStream.Seek(-SizeOf(TBmpHeader), soFromCurrent);
      LoadFromStream(AStream);
      Exit;
    end;

//    if Header.bfType = $5089 then
//    begin
//      AStream.Seek(-SizeOf(TBmpHeader), soFromCurrent);
//      APngImage := TPngImage.Create;
//      try
//        APngImage.LoadFromFile(FileName);
//        SetSize(APngImage.Width, APngImage.Height);
//        Clear($0);
//        APngImage.OldMode := False;
//        APngImage.Draw((FBackend as ICanvasSupport).Canvas, Rect(0, 0, APngImage.Width, APngImage.Height));
//        //Self.SaveToFile('1.bmp');
//      finally
//        APngImage.Free;
//      end;
//      Exit;
//    end;
    ...
    AStream.Position := 0;
    try
      Bmp := JpegDecode(AStream.Memory, AStream.Size);
    except
      Bmp := nil;
    end;
  ...
  Clear(0);
  graphics := TGPGraphics.Create((FBackend as ICanvasSupport).Canvas.Handle);
  try
...
begin
  ...
  begin
    ...
    if C <> 0 then
    begin
      //Gray := (77 * PColor32Entry(P).R + 151 * PColor32Entry(P).G + 28 * PColor32Entry(P).B) shr 8;
      //C := Gray shl 24; // transfer Gray to alpha
      //C := C + Color;
      C := PColor32Entry(P).G shl 24;//P^ shl 24; // transfer blue channel to alpha
      C := C + Color;
      P^ := C;
    end;
    Inc(P);
...
{$ELSE}
        WinApi.Windows.GetTextExtentPoint32W(StockCanvas.Handle, PWideChar(PaddedText),
...
procedure TBitmap32.RenderTextShadown_old(X, Y: Integer; const Text: String; AALevel: Integer; Color: TColor32; ASize: Integer);
var
  B, B2: TBitmap32;
  Sz: TSize;
  Alpha: TColor32;
  PaddedText: String;
  iLoop,
  ScanlineBytes, x1, y1: Integer;
  P1, P2: PColor32EntryArray;
  A, Red, Green, Blue: Byte;
  ABlackColor,
  AWhiteColor: Boolean;
begin
  if Empty then Exit;
  ...
  Alpha := Color shr 24;
  Color := Color and $00FFFFFF;
  Color32ToRGB(Color, Red, Green, Blue);
  AALevel := Constrain(AALevel, -1, 4);
  PaddedText := Text + ' ';

  Font.Quality := fqClearType;
  B := RenderTextBitmap;//TBitmap32.Create;
  B2 := RenderTextBitmap2;//TBitmap32.Create;
  ...
  ScanlineBytes := Integer(B.ScanLine[1]) - Integer(B.ScanLine[0]);
  P1 := PColor32EntryArray(B.ScanLine[0]);
  P2 := PColor32EntryArray(B2.ScanLine[0]);
  ...
  begin
    try
      ...
      begin
        {
        if P1[x1].R > P1[x1].B then
          Alpha := P1[x1].R - P2[x1].R
        else
          Alpha := P1[x1].B - P2[x1].B;
        }
        {
        if B.Font.Color = clWhite then
          A := ((P2[x1].R + P2[x1].G + P2[x1].B)  - (P1[x1].R + P1[x1].G + P1[x1].B)) div 3
        else
          A := ((P1[x1].R + P1[x1].G + P1[x1].B)  - (P2[x1].R + P2[x1].G + P2[x1].B)) div 3;
        if P1[x1].R > P1[x1].B then
          Alpha := P1[x1].R - P2[x1].R
        else
          Alpha := P1[x1].B - P2[x1].B;
        ...
        if P1[x1].A > 0 then
        begin
          P1[x1].R := Red;// * P1[x1].A shr 8;
          P1[x1].G := Green;// * P1[x1].A shr 8;
          P1[x1].B := Blue;// * P1[x1].A shr 8;
        end;
      end;
    finally
  ...
  B.DrawMode := dmBlend;
  B.MasterAlpha := Alpha;
  B.CombineMode := cmMerge;//cmBlend;//Self.CombineMode;

  B.DrawTo(Self, X - ASize, Y);
  B.DrawTo(Self, X + ASize, Y);
  B.DrawTo(Self, X, Y - ASize);
  B.DrawTo(Self, X, Y + ASize);
end;

procedure TBitmap32.RenderText_Old(X, Y: Integer; const Text: String; Color: TColor32; AClearType: Boolean);
var
  B, B2: TBitmap32;
  Sz: TSize;
  Alpha: TColor32;
  ...
  begin
    try
      ...
      begin
        ...
        //A := P1[x1].G - P2[x1].G;
        A := ((P1[x1].R + P1[x1].B)  - (P2[x1].R + P2[x1].B)) SHR 1;
        A := not A;
...
procedure TBitmap32.RenderTextShadown(X, Y: Integer; const Text: String; Color: TColor32; ASize: Integer; AClearType: Boolean);
var
  B, B2: TBitmap32;
  Sz: TSize;
  Alpha: TColor32;
...
begin
  ...
  begin
    RenderText(X, Y, Text, Color, AClearType);
    Exit;
  end;
  ...
  try
    ...
    DrawMode := dmCustom;
    OnPixelCombine := TPixelCombine.PixelCombine;
    ChangeAlpha(Alpha);

    DrawTo(Self, X, Y);
    //DrawTo(Self, X - ASize, Y);
    //DrawTo(Self, X + ASize, Y);
    //DrawTo(Self, X, Y - ASize);
    //DrawTo(Self, X, Y + ASize);
  finally
    //Free;
  end;
...
procedure TBitmap32.RenderText(X, Y: Integer; const Text: String; Color: TColor32; AClearType: Boolean);
var
  BMP: TBitmap32;
  Sz: TSize;
  Alpha: Integer;
  ...
  P: PColor32Array;
  ScanlineBytes, x1, y1, y2, w: integer;
  rgbBlue: Byte;
  rgbGreen: Byte;
  rgbRed: Byte;
  rgbReserved: Byte;
...
begin
  ...
  begin
    ...
    faceDefault := getFreeTypeFace(GR32_TEXT_NORMAL_FONT, GR32_TEXT_BOLD_FONT, GR32_TEXT_HEAVY_FONT, Font);
    if faceDefault = nil then Exit;

    for iLoop := 1 to Text.Length do
    begin
      achar := Text[iLoop];
      ...
      glyph := getFreeTypeGlyph(achar, face, Font);
      if glyph = nil then
      begin
        charIndex := FT_Get_Char_Index(face, Ord(achar));
        if charIndex = 0 then
        begin
          //Continue; //不会有问号符号或X符号

          face := getFreeTypeEmojiFace(GR32_TEXT_EMOJI_FONT, Font);
          if face <> nil then
          begin
            charIndex := FT_Get_Char_Index(face, Ord(achar));
            if charIndex = 0 then
        ...
        if FT_Load_Glyph(face, charIndex, FT_LOAD_RENDER) <> 0 then Exit;
        if FT_Get_Glyph(face^.glyph, @glyph) <> 0 then Exit;

//        if glyph^.format = FT_GLYPH_FORMAT_OUTLINE then
//        begin
//          if FT_Outline_Embolden(@(face^.glyph^.outline), 2 shl 6) <> 0 then
//          begin
//            raise Exception.Create('字体加粗失败...');
//          end;
//        end;
        ...
        begin
          ...
          if Error_ <> 0 then Exit;
        end;
//        if glyph^.format <> FT_GLYPH_FORMAT_BITMAP then
//        begin
//          Error_ := FT_Glyph_To_Bitmap(@glyph, FT_RENDER_MODE_LCD, nil, FT_TRUE);
//          if Error_ <> 0 then Exit;
//        end;

        setFreeTypeGlyph(achar, glyph, Font);
      end;

      glyph_bitmap := FT_BitmapGlyph(glyph);
      bitmap := @(glyph_bitmap^.bitmap);

      ScanlineBytes := Integer(Self.ScanLine[1]) - Integer(Self.ScanLine[0]);
      P := Self.Bits;

      y2 := Y + face.size.metrics.y_ppem - glyph_bitmap^.top;
      if y2 > 0 then
      begin
        if y2 > Self.Height - 1 then Exit;

        inc(Longint(p), ScanlineBytes * y2);
      end;

      w := glyph_bitmap^.root.advance.x shr 16;
      if x + w > Width then Break;

      for y1 := 0 to bitmap.rows - 1 do
      begin
        if y2 >= 0 then
        begin
          for x1 := 0 to bitmap.Width - 1 do
          begin
            if X + x1 + glyph_bitmap^.left < 0 then Continue;
            if X + x1 + glyph_bitmap^.left > Self.Width - 1 then Continue;

            Alpha := PByteArray(bitmap.buffer)[x1 + bitmap.Width * y1];
            Alpha := Round(Alpha * (1.0 + L / 1024));
            if Alpha > 255 then Alpha := 255;

            Color := Color32(R, G, B, Alpha);
            p[X + x1 + glyph_bitmap^.left] := MergeReg(Color, p[X + x1 + glyph_bitmap^.left]);
          end;

          if y2 >= Self.Height - 1 then Break;
          inc(Longint(p), ScanlineBytes);
        end;
...
//      for y1 := 0 to bitmap.rows - 1 do
//      begin
//        for x1 := 0 to bitmap.Width div 3 - 1 do
//        begin
//          R1 := PByteArray(bitmap.buffer)[x1 * 3 + bitmap.pitch * y1];
//          G1 := PByteArray(bitmap.buffer)[x1 * 3 + 1 + bitmap.pitch * y1];
//          B1 := PByteArray(bitmap.buffer)[x1 * 3 + 2 + bitmap.pitch * y1];
//          RGBtoHSL(Color32(R1, G1, B1, 255), H, S, L);
//
//          Alpha := L;
//          Color := Color32(R, G, B, Alpha);
//          p[X + x1 + glyph_bitmap^.left] := MergeReg(Color, p[X + x1 + glyph_bitmap^.left]);
...
//      end;
  ...
  BMP := RenderTextBitmap;//TBitmap32.Create;
  with BMP do
  try
    Font := Self.Font;
    Sz := TextExtent(PaddedText);
    ...
    Font.Color := clWhite;
    Textout(0, 0, Text);
    TextBlueToAlpha(BMP, Color);

    DrawMode := dmBlend;
    MasterAlpha := 255;//Alpha;
    if AClearType then
    begin
      DrawMode := dmCustom;
      OnPixelCombine := TPixelCombine.PixelCombine;
      //CombineMode := cmMerge;//Self.CombineMode;
    end
    else
    begin
      DrawMode := dmCustom;
      OnPixelCombine := TPixelCombine.PixelCombine;
      //CombineMode := cmBlend;//Self.CombineMode;
    end;
...
function TBitmap32.TextExtentEx(const Text: String): TSize;
var
  StockCanvas: TCanvas;
begin
  StockCanvas := StockBitmap.Canvas;
  ...
  try
    StockCanvas.Font := Font;
    Result := StockCanvas.TextExtent(Text);
  finally
    StockCanvas.Unlock;
  end;
end;
...
//        end;
      ...
      glyph_bitmap := FT_BitmapGlyph(glyph);
      bitmap := @(glyph_bitmap^.bitmap);
      Result.Width := Result.Width + glyph_bitmap^.root.advance.x shr 16;
      if face.size.metrics.y_ppem + Trunc(face.size.metrics.y_ppem / 6) > Result.Height then
      begin
        Result.Height := face.size.metrics.y_ppem + Trunc(face.size.metrics.y_ppem / 6);
      end;
    end;
    Exit;
  end;

  Result := TextExtentEx(Text);
...
initialization
  ...
  StockBitmap.Width := 208;
  StockBitmap.Height := 28;
  RenderTextBitmap := TBitmap32.Create;
  RenderTextBitmap2 := TBitmap32.Create;

  //initFreeType;

finalization
  RenderTextBitmap2.Free;
  RenderTextBitmap.Free;
  StockBitmap.Free;
