...
begin
  ...
  Self.cbAnimateCloseStyle.ItemIndex := 9;

end;

procedure TDemoForm.InitEdit;
begin
  edFontSize.Items.Add.Caption := 'Ð¡ºÅÎÄ×Ö';
  edFontSize.Items.Add.Caption := 'ÖÐºÅÎÄ×Ö';
  edFontSize.Items.Add.Caption := '´óºÅÎÄ×Ö';
  edFontSize.ItemIndex := 0;

  cbLineSpace.Items.Add.Caption := '1.0 ±¶ÐÐ¾à';
  cbLineSpace.Items.Add.Caption := '1.2 ±¶ÐÐ¾à';
  cbLineSpace.Items.Add.Caption := '1.4 ±¶ÐÐ¾à';
  cbLineSpace.Items.Add.Caption := '1.6 ±¶ÐÐ¾à';
  cbLineSpace.Items.Add.Caption := '1.8 ±¶ÐÐ¾à';
  cbLineSpace.Items.Add.Caption := '2.0 ±¶ÐÐ¾à';
  cbLineSpace.Items.Add.Caption := '2.2 ±¶ÐÐ¾à';
  cbLineSpace.Items.Add.Caption := '2.4 ±¶ÐÐ¾à';
  cbLineSpace.Items.Add.Caption := '2.6 ±¶ÐÐ¾à';
  cbLineSpace.Items.Add.Caption := '2.8 ±¶ÐÐ¾à';
  cbLineSpace.Items.Add.Caption := '3.0 ±¶ÐÐ¾à';
...
begin
  ...
  try
    ...
    for iLoop := 1 to 5 do
    begin
      AddSkin('Í¼Æ¬Æ¤·ô' + IntToStr(iLoop),
              ExtractFilePath(Application.ExeName) + 'skins\skin' + IntToStr(iLoop) + '.jpg',
              0,
              bdtStretch);
    end;
    ...
    AddSkin('´¿É«Æ¤·ô(#29B6F6)',
            ...
            $FF29B6F6,
            bdtStretch);
    AddSkin('´¿É«Æ¤·ô(#9575CD)',
            '',
            $FF9575CD,
            bdtStretch);
    AddSkin('´¿É«Æ¤·ô(#F06292)',
            '',
            $FFF06292,
            bdtStretch);
    AddSkin('´¿É«Æ¤·ô(#8BC34A)',
            '',
            $FF8BC34A,
            bdtStretch);
    AddSkin('´¿É«Æ¤·ô(#4DB6AC)',
            '',
            $FF4DB6AC,
            bdtStretch);
    AddSkin('´¿É«Æ¤·ô(#FFA000)',
            '',
            $FFFFA000,
            bdtStretch);

    for ASkin in FSkins do cbSkins.Items.Add.Caption := ASkin.FSkinName;
...
begin
  jduMessageBox(Handle, ' ÄãºÃ£¬»¶Ó­¹âÁÙ£¡', 'ÌáÊ¾',  MB_OK);
end;

procedure TDemoForm.JDUIButton2Click(Sender: TObject);
begin
  jduMessageBox(Handle, ' ÄãºÃ£¬»¶Ó­¹âÁÙ£¡', 'ÌáÊ¾',  MB_OKCANCEL);
end;

procedure TDemoForm.JDUIButton3Click(Sender: TObject);
begin
  jduMessageBox(Handle, ' ÄãºÃ£¬»¶Ó­¹âÁÙ£¡', 'ÌáÊ¾',  MB_YESNO);
end;

procedure TDemoForm.JDUIButton4Click(Sender: TObject);
begin
  jduMessageBox(Handle, ' ÄãºÃ£¬»¶Ó­¹âÁÙ£¡', 'ÌáÊ¾',  MB_ABORTRETRYIGNORE);
end;
...
begin
  ...
  try
    ...
    jduPage0.Visible := jduTab0.Down;
    jduPage1.Visible := jduTab1.Down;
    jduPage2.Visible := jduTab2.Down;
    jduPage3.Visible := jduTab3.Down;
    jduPage4.Visible := jduTab4.Down;
    jduPage5.Visible := jduTab5.Down;
    jduPage6.Visible := jduTab6.Down;
    if jduPage2.Visible then
    begin
    ...
    if jduPage3.Visible then
    begin
      jduGridView.ReAlignAll;
    end;
    if jduPage5.Visible then
    begin
