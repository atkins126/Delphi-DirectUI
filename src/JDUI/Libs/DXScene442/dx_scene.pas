...
uses
  ...
  {$ENDIF}
  {$IFDEF WINDOWS}
  WinApi.Windows, WinApi.Messages, WinApi.ShellAPI, WinApi.ActiveX, WinApi.CommCtrl, WinApi.MMSystem,
  {$ENDIF}
  {$IFDEF FPC}
...
type
  ...
  public
    ...
    property BufferDC: Cardinal read FBufferDC;
  published
  end;
  ...
  public
    ...
    procedure AnimateFloat(const APropertyName: string; const NewValue: single; Duration: single = 0.2;
      AType: TvxAnimationType = vgAnimationIn; AInterpolation: TvxInterpolationType = vgInterpolationLinear; AFinishedCallBack: TNotifyEvent = nil);
  ...
  published
    ...
    property OnUpdateLayer: TNotifyEvent read FOnUpdateLayer write FOnUpdateLayer;
  end;
...
uses
  ...
  dx_opengl,
  {$endif}
  dx_version, dx_ani, dx_objects, dx_resource, dx_physics_newton, dx_textbox, dx_controls, dx_vglayer;
      ...
      kEventControlDragWithin:
        begin
          ...
          begin
            ...
            begin
              if TdxScene(inUserData).Root = nil then Exit;
...
begin
  ...
  dx_utils.InvertMatrix(FCurrentCameraInvMatrix);
end;
...
begin
  ...
  begin
    ...
    //outputdebugstring(pchar(format('FTime: %f, FDuration: %f', [FTime, FDuration])));
  end
  else
    Result := 0;
end;
...
begin
  if (Parent <> nil) and (Parent.IsVisual) and (not TdxVisualObject(Parent).Visible) then Exit;
...
begin
  ...
  try
    ...
    while (not FDestroying) and A.FRunning and (GetTickCount - iTicket < Duration * 1000 * 2) do
    begin
...
begin
  ...
  if FChildren <> nil then
    for i := 0 to FChildren.Count - 1 do
    begin
      if not TdxObject(FChildren[i]).isVisual then Continue;
      Child := FChildren[i];
  ...
  try
    ...
    begin
      ...
      if FScene.FChildren <> nil then
        ...
        begin
          if not (TObject(FScene.FChildren[i]) is TdxVisualObject) then Continue;
          if not TdxVisualObject(FScene.FChildren[i]).Visible then Continue;
...
var
  ...
  begin
    ...
    begin
      ...
      begin
        J := 0;
        while (J < AlignList.Count) and not InsertBefore(Control, TdxVisualObject(AlignList[J]), AAlign) do Inc(J);
...
var
  ...
  begin
    ...
    for I := 0 to ChildrenCount - 1 do
    begin
      if not (Children[i] is TdxCustomLayer) then Continue;
      ...
      begin
        J := 0;
        while (J < AlignList.Count) and not InsertBefore(Control, TdxCustomLayer(AlignList[J]), AAlign) do Inc(J);
        AlignList.Insert(J, Control);
      end;
    end;
    for I := 0 to AlignList.Count - 1 do
...
begin
  ...
  PrevWndProc := WinApi.Windows.WNDPROC(SetWindowLongPtrW(Self.Handle, GWL_WNDPROC, PtrInt(@WndCallback)));
  {$ENDIF}
...
begin
  ...
  begin
    WinApi.Windows.InvalidateRect(Handle, nil, false);
  end;
  {$ENDIF}
  {$IFDEF DARWIN}
...
begin
  ...
  try
    ...
    if FChildren <> nil then
      for i := 0 to FChildren.Count - 1 do
      begin
        if not (TObject(FChildren[i]) is TdxVisualObject) then Continue;
        if not TdxVisualObject(FChildren[i]).Visible then Continue;
...
var
  ...
  Origin, Size, BitmapOrigin: WinApi.Windows.TPoint;
  dragAlpha: integer;
...
begin
  ...
  if Assigned(FOnUpdateLayer) then
    FOnUpdateLayer(Self)
  else
...
begin
  ...
  begin
    Obj := Children[i];
    if not (Obj is TdxVisualObject) then Exit;
    if Obj.Tag = $FFFe then Continue;
    if not TdxVisualObject(Obj).Visible and (not DesignTime) then Continue;
    if (TdxVisualObject(Obj).Locked) and (DesignTime) then Continue;
...
Exit;
  if (DesignSnapToLines) and (FSelected.Parent <> nil) then
    ...
    begin
      if TdxObject(FSelected.Parent.FChildren[i]) = FSelected then Continue;
      if not TdxObject(FSelected.Parent.FChildren[i]).isVisual then Continue;
...
(*  W := ARect.Right - ARect.Left;
  ...
  begin
    ...
    if WinApi.Windows.FindResource(HInstance, PChar(RName), RT_RCDATA) <> 0 then
    begin
