...
type
  TvxCustomSynEdit = class(TvxScrollBox)
  private
  private
    ...
    fChainedEditor: TvxCustomSynEdit;
    ...
    procedure PluginsAfterPaint(ACanvas: TvxCanvas; const AClip: TvxRect;
  ...
  protected
    ...
    procedure InvalidateRect(aRect: TvxRect; aErase: Boolean); virtual;
    ...
    procedure PaintGutter(const AClip: TvxRect; const aFirstRow,
      aLastRow: Integer); virtual;
    procedure PaintTextLines(AClip: TvxRect; const aFirstRow, aLastRow,
  ...
  public
    ...
    function RowColumnToPixels(const RowCol: TDisplayCoord): TvxPoint;
    ...
    procedure SetLinesPointer(ASynEdit: TvxCustomSynEdit);
  ...
  TvxSynEdit = class(TvxCustomSynEdit)
  published
...
function TvxCustomSynEdit.PixelsToNearestRowColumn(aX, aY: Integer): TDisplayCoord;
...
function TvxCustomSynEdit.PixelsToRowColumn(aX, aY: Integer): TDisplayCoord;
begin
  Result.Column := trunc(Max(1, LeftChar + ((aX - fGutterWidth - 2) / fCharWidth)));
  Result.Row := trunc(Max(1, TopLine + (aY / fTextHeight)));
end;

function TvxCustomSynEdit.RowColumnToPixels(const RowCol: TDisplayCoord): TvxPoint;
begin
  Result.X := (RowCol.Column-1) * fCharWidth + fTextOffset;
  Result.Y := (RowCol.Row - fTopLine) * fTextHeight;
end;

procedure TvxCustomSynEdit.ComputeCaret(X, Y: Integer);
...
procedure TvxCustomSynEdit.ComputeScroll(X, Y: Integer);
//X,Y are pixel coordinates
var
  iScrollBounds: TvxRect; { relative to the client area }
begin
...
procedure TvxCustomSynEdit.DoCopyToClipboard(const SText: WideString);
var
  Data: THandle;
  DataPtr: Pointer;
  Size: Cardinal;
...
procedure TvxCustomSynEdit.CopyToClipboard;
var
  SText: WideString;
  ChangeTrim: Boolean;
begin
...
procedure TvxCustomSynEdit.CutToClipboard;
begin
...
constructor TvxCustomSynEdit.Create(AOwner: TComponent);
begin
  ...
  FSelectionFill := TvxBrush.Create(vgBrushSolid, $40002070);
...
procedure TvxCustomSynEdit.DecPaintLock;
var
...
destructor TvxCustomSynEdit.Destroy;
begin
  Highlighter := nil;
...
function TvxCustomSynEdit.GetBlockBegin: TBufferCoord;
begin
...
function TvxCustomSynEdit.GetBlockEnd: TBufferCoord;
begin
  if (fBlockEnd.Line < fBlockBegin.Line)
    or ((fBlockEnd.Line = fBlockBegin.Line) and (fBlockEnd.Char < fBlockBegin.Char))
  then
...
procedure TvxCustomSynEdit.SynFontChanged(Sender: TObject);
begin
  RecalcCharExtent;
  SizeOrFontChanged(True);
end;

function TvxCustomSynEdit.GetLineText: WideString;
begin
...
function TvxCustomSynEdit.GetSelText: WideString;
...
function TvxCustomSynEdit.SynGetText: WideString;
begin
  Result := Lines.Text;
end;

function TvxCustomSynEdit.GetWordAtCursor: WideString;
var
  bBegin: TBufferCoord;
  bEnd: TBufferCoord;
begin
...
procedure TvxCustomSynEdit.HideCaret;
begin
...
procedure TvxCustomSynEdit.IncPaintLock;
begin
  inc(fPaintLock);
end;

procedure TvxCustomSynEdit.InvalidateGutter;
begin
  InvalidateGutterLines(-1, -1);
end;

procedure TvxCustomSynEdit.InvalidateGutterLine(aLine: Integer);
begin
...
procedure TvxCustomSynEdit.InvalidateGutterLines(FirstLine, LastLine: integer);
...
procedure TvxCustomSynEdit.InvalidateLines(FirstLine, LastLine: integer);
// note: FirstLine and LastLine don't need to be in correct order
var
  rcInval: TvxRect;
begin
...
procedure TvxCustomSynEdit.InvalidateSelection;
begin
...
procedure TvxCustomSynEdit.KeyUp(var Key: Word; Shift: TShiftState);
begin
...
procedure TvxCustomSynEdit.KeyDown(var Key: Word; var KeyChar: System.WideChar; Shift: TShiftState);
var
  Data: pointer;
  C: WideChar;
...
procedure TvxCustomSynEdit.Loaded;
begin
  inherited Loaded;
  GutterChanged(Self);
  UpdateScrollBars;
...
procedure TvxCustomSynEdit.KeyPress(var Key: WideChar);
begin
...
function TvxCustomSynEdit.LeftSpaces(const Line: WideString): Integer;
begin
  Result := LeftSpacesEx(Line, False);
end;

function TvxCustomSynEdit.LeftSpacesEx(const Line: WideString; WantTabs: Boolean): Integer;
var
  p: PWideChar;
begin
  p := PWideChar(Line);
...
function TvxCustomSynEdit.GetLeftSpacing(CharCount: Integer; WantTabs: Boolean): WideString;
begin
...
procedure TvxCustomSynEdit.LinesChanging(Sender: TObject);
begin
  Include(fStateFlags, sfLinesChanging);
end;

procedure TvxCustomSynEdit.LinesChanged(Sender: TObject);
var
...
begin
  ...
  begin
    ...
    FillChar(fInvalidateRect, SizeOf(TvxRect), 0);
...
procedure TvxCustomSynEdit.EnterFocus;
begin
  inherited;
  ShowCaret;
  UpdateCaret;
...
procedure TvxCustomSynEdit.Killfocus;
begin
  inherited;
  HideCaret;
end;

procedure TvxCustomSynEdit.MouseDown(Button: TMouseButton; Shift: TShiftState; X, Y: single);
var
  bWasSel: Boolean;
  bStartDrag: Boolean;
...
procedure TvxCustomSynEdit.MouseMove(Shift: TShiftState; X, Y, Dx, Dy: single);
var
  P: TDisplayCoord;
begin
...
procedure TvxCustomSynEdit.MouseUp(Button: TMouseButton; Shift: TShiftState; X, Y: single);
begin
...
procedure TvxCustomSynEdit.DoOnGutterClick(Button: TMouseButton; X, Y: Integer);
var
  i     : Integer;
  offs  : Integer;
  line  : Integer;
...
procedure TvxCustomSynEdit.Realign;
begin
  inherited;
...
procedure TvxCustomSynEdit.ApplyStyle;
var
  T: TvxObject;
begin
  inherited;
...
procedure TvxCustomSynEdit.UpdateScrollBars;
var
  nMaxScroll: Integer;
  iClientRect: TvxRect;
begin
...
procedure TvxCustomSynEdit.VScrollChange(Sender: TObject);
begin
  inherited ;
...
procedure TvxCustomSynEdit.HScrollChange(Sender: TObject);
begin
  inherited ;
...
function TvxCustomSynEdit.GetContentBounds: TvxRect;
var
...
procedure TvxCustomSynEdit.DoContentPaint(Sender: TObject; const Canvas: TvxCanvas; const ARect: TvxRect);
var
  rcClip, rcDraw: TvxRect;
...
procedure TvxCustomSynEdit.PaintGutter(const AClip: TvxRect;
...
// multi-byte chars.
function TvxCustomSynEdit.ExpandAtWideGlyphs(const S: UnicodeString): UnicodeString;
var
...
function TvxCustomSynEdit.ShrinkAtWideGlyphs(const S: WideString; First: Integer;
  var CharCount: Integer): WideString;
var
  i, j: Integer;
begin
...
procedure TvxCustomSynEdit.PaintTextLines(AClip: TvxRect; const aFirstRow, aLastRow,
...
//        or (not (fsUnderline in Style) and not (fsUnderline in TokenAccu.Style) and TokenIsSpaces)
  ...
  function FontStyleToVG(Style: TFontStyles): TvxFontStyle;
  begin
...
procedure TvxCustomSynEdit.PasteFromClipboard;
var
...
procedure TvxCustomSynEdit.SelectAll;
var
  LastPt: TBufferCoord;
begin
  LastPt.Char := 1;
...
procedure TvxCustomSynEdit.SetBlockBegin(Value: TBufferCoord);
var
...
procedure TvxCustomSynEdit.SetBlockEnd(Value: TBufferCoord);
var
  nLine: Integer;
begin
  ActiveSelectionMode := SelectionMode;
...
procedure TvxCustomSynEdit.SetCaretX(Value: Integer);
var
...
procedure TvxCustomSynEdit.SetCaretY(Value: Integer);
var
  vNewCaret: TBufferCoord;
begin
...
procedure TvxCustomSynEdit.InternalSetCaretX(Value: Integer);
var
  vNewCaret: TBufferCoord;
begin
  vNewCaret.Char := Value;
...
procedure TvxCustomSynEdit.InternalSetCaretY(Value: Integer);
var
  vNewCaret: TBufferCoord;
begin
  vNewCaret.Line := Value;
...
function TvxCustomSynEdit.GetCaretXY: TBufferCoord;
begin
  Result.Char := CaretX;
  Result.Line := CaretY;
end;

function TvxCustomSynEdit.GetDisplayX: Integer;
begin
  Result := DisplayXY.Column;
end;

function TvxCustomSynEdit.GetDisplayY: Integer;
begin
...
Function TvxCustomSynEdit.GetDisplayXY: TDisplayCoord;
begin
...
procedure TvxCustomSynEdit.SetCaretXY(const Value: TBufferCoord);
...
procedure TvxCustomSynEdit.InternalSetCaretXY(const Value: TBufferCoord);
begin
  SetCaretXYEx(True, Value);
end;

procedure TvxCustomSynEdit.UpdateLastCaretX;
begin
  fLastCaretX := DisplayX;
end;

procedure TvxCustomSynEdit.SetCaretXYEx(CallEnsureCursorPos: Boolean; Value: TBufferCoord);
var
  nMaxX: Integer;
...
function TvxCustomSynEdit.CaretInView: Boolean;
var
...
procedure TvxCustomSynEdit.SetActiveLineColor(Value: TColor);
begin
...
procedure TvxCustomSynEdit.SetGutterWidth(Value: Integer);
begin
  Value := Max(Value, 0);
...
procedure TvxCustomSynEdit.SetLeftChar(Value: Integer);
var
  MaxVal: Integer;
  iDelta: Integer;
  iTextArea: TvxRect;
...
procedure TvxCustomSynEdit.SetLines(Value: TUnicodeStrings);
begin
  Lines.Assign(Value);
end;

procedure TvxCustomSynEdit.SetLineText(Value: WideString);
begin
  if (CaretY >= 1) and (CaretY <= Max(1, Lines.Count)) then
    Lines[CaretY - 1] := Value;
end;

procedure TvxCustomSynEdit.SetName(const Value: TComponentName);
var
  TextToName: Boolean;
begin
...
procedure TvxCustomSynEdit.SetSelTextPrimitive(const Value: WideString);
begin
...
procedure TvxCustomSynEdit.SetSelTextPrimitiveEx(PasteMode: TSynSelectionMode;
...
procedure TvxCustomSynEdit.SynSetText(const Value: WideString);
begin
  Lines.Text := Value;
end;

procedure TvxCustomSynEdit.SetTopLine(Value: Integer);
var
  Delta: Integer;
begin
...
procedure TvxCustomSynEdit.ShowCaret;
begin
...
procedure TvxCustomSynEdit.UpdateCaret;
var
  CX, CY: single;
  iClientRect: TvxRect;
...
function TvxCustomSynEdit.ScanFrom(Index: Integer): Integer;
var
...
procedure TvxCustomSynEdit.ListCleared(Sender: TObject);
begin
  if WordWrap then
...
procedure TvxCustomSynEdit.ListDeleted(Sender: TObject; aIndex: Integer;
  aCount: Integer);
begin
...
procedure TvxCustomSynEdit.ListInserted(Sender: TObject; Index: Integer;
  aCount: Integer);
var
  L: Integer;
  vLastScan: Integer;
...
procedure TvxCustomSynEdit.ListPutted(Sender: TObject; Index: Integer;
  aCount: Integer);
var
  L: Integer;
  vEndLine: Integer;
...
procedure TvxCustomSynEdit.ScanRanges;
var
  i: Integer;
begin
...
procedure TvxCustomSynEdit.SetWordBlock(Value: TBufferCoord);
var
...
procedure TvxCustomSynEdit.DblClick;
var
  ptMouse: TvxPoint;
begin
...
function TvxCustomSynEdit.GetCanUndo: Boolean;
begin
  result := not ReadOnly and fUndoList.CanUndo;
end;

function TvxCustomSynEdit.GetCanRedo: Boolean;
begin
  result := not ReadOnly and fRedoList.CanUndo;
end;

function TvxCustomSynEdit.GetCanPaste;
begin
//  Result := not ReadOnly and ClipboardProvidesText;
end;

procedure TvxCustomSynEdit.InsertBlock(const BB, BE: TBufferCoord; ChangeStr: PWideChar;
  AddToUndoList: Boolean);
...
procedure TvxCustomSynEdit.Redo;
...
procedure TvxCustomSynEdit.RedoItem;
var
  Item: TSynEditUndoItem;
...
procedure TvxCustomSynEdit.Undo;
...
procedure TvxCustomSynEdit.UndoItem;
var
  Item: TSynEditUndoItem;
  TmpPos: TBufferCoord;
  TmpStr: WideString;
        ...
        begin
          ...
          if Item.ChangeSelMode = smColumn then
            ...
            TmpPos := TBufferCoord(vgMinPoint(
              vgPoint(Item.ChangeStartPos.Char, Item.ChangeStartPos.Line), TvxPoint(Item.ChangeEndPos)));
...
procedure TvxCustomSynEdit.ClearBookMark(BookMark: Integer);
begin
...
procedure TvxCustomSynEdit.GotoBookMark(BookMark: Integer);
var
...
procedure TvxCustomSynEdit.GotoLineAndCenter(ALine: Integer);
begin
...
procedure TvxCustomSynEdit.SetBookMark(BookMark: Integer; X: Integer; Y: Integer);
var
  mark: TSynEditMark;
begin
...
procedure TvxCustomSynEdit.ChainListCleared(Sender: TObject);
begin
...
procedure TvxCustomSynEdit.ChainListDeleted(Sender: TObject; aIndex: Integer;
  aCount: Integer);
begin
...
procedure TvxCustomSynEdit.ChainListInserted(Sender: TObject; aIndex: Integer;
  aCount: Integer);
begin
...
procedure TvxCustomSynEdit.ChainListPutted(Sender: TObject; aIndex: Integer;
  aCount: Integer);
begin
...
procedure TvxCustomSynEdit.ChainLinesChanging(Sender: TObject);
begin
...
procedure TvxCustomSynEdit.ChainLinesChanged(Sender: TObject);
begin
...
procedure TvxCustomSynEdit.ChainUndoRedoAdded(Sender: TObject);
var
...
procedure TvxCustomSynEdit.UnHookTextBuffer;
var
  vOldWrap: Boolean;
begin
...
procedure TvxCustomSynEdit.HookTextBuffer(aBuffer: TSynEditStringList;
  aUndo, aRedo: TSynEditUndoList);
var
  vOldWrap: Boolean;
begin
...
procedure TvxCustomSynEdit.LinesHookChanged;
var
...
procedure TvxCustomSynEdit.SetLinesPointer(ASynEdit: TvxCustomSynEdit);
begin
...
procedure TvxCustomSynEdit.RemoveLinesPointer;
begin
...
{$IFDEF SYN_CLX}
function TvxCustomSynEdit.EventFilter(Sender: QObjectH; Event: QEventH): Boolean;
begin
...
procedure TvxCustomSynEdit.DragCanceled;
begin
  inherited;
end;

procedure TvxCustomSynEdit.SetRightEdge(Value: Integer);
begin
...
procedure TvxCustomSynEdit.SetRightEdgeColor(Value: TColor);
var
  nX: Integer;
  rcInval: TvxRect;
begin
...
function TvxCustomSynEdit.GetMaxUndo: Integer;
begin
  result := fUndoList.MaxUndoActions;
end;

procedure TvxCustomSynEdit.SetMaxUndo(const Value: Integer);
begin
  if Value > -1 then
  begin
...
procedure TvxCustomSynEdit.Notification(AComponent: TComponent;
...
procedure TvxCustomSynEdit.SetHighlighter(const Value: TSynCustomHighlighter);
begin
...
procedure TvxCustomSynEdit.SetHideSelection(const Value: Boolean);
begin
...
procedure TvxCustomSynEdit.SetInsertMode(const Value: Boolean);
begin
...
procedure TvxCustomSynEdit.InitializeCaret;
var
...
procedure TvxCustomSynEdit.SetInsertCaret(const Value: TSynEditCaretType);
begin
...
procedure TvxCustomSynEdit.SetOverwriteCaret(const Value: TSynEditCaretType);
begin
...
procedure TvxCustomSynEdit.SetMaxScrollWidth(Value: Integer);
begin
...
procedure TvxCustomSynEdit.EnsureCursorPosVisible;
begin
  EnsureCursorPosVisibleEx(False);
end;

procedure TvxCustomSynEdit.EnsureCursorPosVisibleEx(ForceToMiddle: Boolean);
var
  TmpMiddle: Integer;
  VisibleX: Integer;
  vCaretRow: Integer;
...
procedure TvxCustomSynEdit.SetKeystrokes(const Value: TSynEditKeyStrokes);
begin
  if Value = nil then
    FKeystrokes.Clear
  else
...
procedure TvxCustomSynEdit.SetDefaultKeystrokes;
begin
...
function TvxCustomSynEdit.TranslateKeyCode(Code: word; Shift: TShiftState;
  var Data: pointer): TSynEditorCommand;
var
  i: Integer;
...
procedure TvxCustomSynEdit.CommandProcessor(Command: TSynEditorCommand;
...
procedure TvxCustomSynEdit.ExecuteCommand(Command: TSynEditorCommand; AChar: WideChar;
  Data: pointer);
...
procedure TvxCustomSynEdit.DoOnCommandProcessed(Command: TSynEditorCommand;
  AChar: WideChar; Data: pointer);
begin
...
procedure TvxCustomSynEdit.DoOnProcessCommand(var Command: TSynEditorCommand;
...
procedure TvxCustomSynEdit.ClearAll;
begin
  Lines.Clear;
...
procedure TvxCustomSynEdit.ClearSelection;
begin
  if SelAvail then
    SelText := '';
end;

function TvxCustomSynEdit.NextWordPosEx(const XY: TBufferCoord): TBufferCoord;
var
...
function TvxCustomSynEdit.WordStartEx(const XY: TBufferCoord): TBufferCoord;
var
  CX, CY: Integer;
  Line: WideString;
begin
...
function TvxCustomSynEdit.WordEndEx(const XY: TBufferCoord): TBufferCoord;
var
  CX, CY: Integer;
  Line: WideString;
begin
...
function TvxCustomSynEdit.PrevWordPosEx(const XY: TBufferCoord): TBufferCoord;
var
  CX, CY: Integer;
  Line: WideString;
begin
...
procedure TvxCustomSynEdit.SetSelectionMode(const Value: TSynSelectionMode);
begin
...
procedure TvxCustomSynEdit.SetActiveSelectionMode(const Value: TSynSelectionMode);
begin
...
procedure TvxCustomSynEdit.SetAdditionalIdentChars(const Value: TSysCharSet);
begin
  FAdditionalIdentChars := Value;
end;

procedure TvxCustomSynEdit.SetAdditionalWordBreakChars(const Value: TSysCharSet);
begin
  FAdditionalWordBreakChars := Value;
end;

procedure TvxCustomSynEdit.BeginUndoBlock;
begin
  fUndoList.BeginBlock;
end;

procedure TvxCustomSynEdit.BeginUpdate;
begin
  IncPaintLock;
end;

procedure TvxCustomSynEdit.EndUndoBlock;
begin
  fUndoList.EndBlock;
end;

procedure TvxCustomSynEdit.EndUpdate;
begin
  DecPaintLock;
end;

procedure TvxCustomSynEdit.AddKey(Command: TSynEditorCommand;
...
procedure TvxCustomSynEdit.MarkListChange(Sender: TObject);
begin
  InvalidateGutter;
end;

function TvxCustomSynEdit.GetSelStart: integer;
begin
  if GetSelAvail then
...
procedure TvxCustomSynEdit.SetAlwaysShowCaret(const Value: Boolean);
begin
...
procedure TvxCustomSynEdit.SetSelStart(const Value: Integer);
begin
...
function TvxCustomSynEdit.GetSelEnd: Integer;
begin
  if GetSelAvail then
...
procedure TvxCustomSynEdit.SetSelEnd(const Value: Integer);
begin
  BlockEnd := CharIndexToRowCol( Value );
end;

procedure TvxCustomSynEdit.SetSelWord;
begin
  SetWordBlock(CaretXY);
end;

procedure TvxCustomSynEdit.SetExtraLineSpacing(const Value: Integer);
begin
  fExtraLineSpacing := Value;
  SynFontChanged(self);
end;

function TvxCustomSynEdit.GetBookMark(BookMark: Integer; var X, Y: Integer):
  Boolean;
var
  i: Integer;
begin
...
function TvxCustomSynEdit.IsBookmark(BookMark: Integer): Boolean;
var
  x, y: Integer;
begin
...
procedure TvxCustomSynEdit.ClearUndo;
begin
  fUndoList.Clear;
  fRedoList.Clear;
end;

procedure TvxCustomSynEdit.SetSelTextExternal(const Value: WideString);
var
...
procedure TvxCustomSynEdit.SetGutter(const Value: TSynGutter);
begin
  fGutter.Assign(Value);
end;

procedure TvxCustomSynEdit.GutterChanged(Sender: TObject);
var
  nW: Integer;
begin
...
procedure TvxCustomSynEdit.LockUndo;
begin
  fUndoList.Lock;
  fRedoList.Lock;
end;

procedure TvxCustomSynEdit.UnlockUndo;
begin
  fUndoList.Unlock;
  fRedoList.Unlock;
end;

procedure TvxCustomSynEdit.SetTabWidth(Value: Integer);
begin
...
procedure TvxCustomSynEdit.SelectedColorsChanged(Sender: TObject);
begin
  InvalidateSelection;
end;
...
function TvxCustomSynEdit.SearchReplace(const ASearch, AReplace: WideString;
  AOptions: TSynSearchOptions): Integer;
var
...
function TvxCustomSynEdit.IsPointInSelection(const Value: TBufferCoord): boolean;
var
...
procedure TvxCustomSynEdit.SetFocus;
begin
...
procedure TvxCustomSynEdit.BookMarkOptionsChanged(Sender: TObject);
begin
  InvalidateGutter;
end;

function TvxCustomSynEdit.GetOptions: TSynEditorOptions;
begin
  Result := fOptions;
end;

procedure TvxCustomSynEdit.SetOptions(Value: TSynEditorOptions);
const
...
procedure TvxCustomSynEdit.SizeOrFontChanged(bFont: boolean);
begin
...
procedure TvxCustomSynEdit.MoveCaretHorz(DX: Integer; SelectionCommand: Boolean);
var
...
procedure TvxCustomSynEdit.MoveCaretVert(DY: Integer; SelectionCommand: Boolean);
var
...
procedure TvxCustomSynEdit.MoveCaretAndSelection(const ptBefore, ptAfter: TBufferCoord;
  SelectionCommand: Boolean);
begin
...
procedure TvxCustomSynEdit.SetCaretAndSelection(const ptCaret, ptBefore,
...
procedure TvxCustomSynEdit.RecalcCharExtent;
{const
  iFontStyles: array[0..3] of TvxFontStyle = ([], vgFontItalic, vgFontBold,
    [fsItalic, fsBold]);
var
...
procedure TvxCustomSynEdit.HighlighterAttrChanged(Sender: TObject);
begin
  RecalcCharExtent;
...
procedure TvxCustomSynEdit.StatusChanged(AChanges: TSynStatusChanges);
begin
...
procedure TvxCustomSynEdit.DoCaseChange(const Cmd: TSynEditorCommand);
...
procedure TvxCustomSynEdit.DoTabKey;
var
  StartOfBlock: TBufferCoord;
...
procedure TvxCustomSynEdit.DoShiftTabKey;
...
procedure TvxCustomSynEdit.DoHomeKey(Selection: Boolean);
...
procedure TvxCustomSynEdit.DoEndKey(Selection: Boolean);
...
procedure TvxCustomSynEdit.InvalidateRect(aRect: TvxRect; aErase: Boolean);
begin
...
procedure TvxCustomSynEdit.DoBlockIndent;
var
...
procedure TvxCustomSynEdit.DoBlockUnindent;
var
  OrgCaretPos,
  BB, BE: TBufferCoord;
  Line, Run,
...
function TvxCustomSynEdit.ExecuteAction(Action: TBasicAction): Boolean;
begin
...
function TvxCustomSynEdit.UpdateAction(Action: TBasicAction): Boolean;
begin
  if Action is TEditAction then
  begin
    Result := IsFocused;
...
procedure TvxCustomSynEdit.SetModified(Value: Boolean);
begin
...
function TvxCustomSynEdit.DoOnSpecialLineColors(Line: Integer; var Foreground,
...
procedure TvxCustomSynEdit.InvalidateLine(Line: Integer);
var
  rcInval: TvxRect;
begin
...
function TvxCustomSynEdit.GetReadOnly: Boolean;
begin
  Result := fReadOnly;
end;

procedure TvxCustomSynEdit.SetReadOnly(Value: Boolean);
begin
...
procedure TvxCustomSynEdit.FindMatchingBracket;
begin
  InternalCaretXY := GetMatchingBracket;
end;

function TvxCustomSynEdit.GetMatchingBracket: TBufferCoord;
begin
  Result := GetMatchingBracketEx(CaretXY);
end;

function TvxCustomSynEdit.GetMatchingBracketEx(const APoint: TBufferCoord): TBufferCoord;
const
...
function TvxCustomSynEdit.GetHighlighterAttriAtRowCol(const XY: TBufferCoord;
...
function TvxCustomSynEdit.GetHighlighterAttriAtRowColEx(const XY: TBufferCoord;
...
function TvxCustomSynEdit.FindHookedCmdEvent(AHandlerProc: THookedCommandEvent): Integer;
var
...
function TvxCustomSynEdit.GetHookedCommandHandlersCount: Integer;
begin
...
procedure TvxCustomSynEdit.RegisterCommandHandler(
...
procedure TvxCustomSynEdit.UnregisterCommandHandler(AHandlerProc:
  THookedCommandEvent);
var
  i: Integer;
begin
...
procedure TvxCustomSynEdit.NotifyHookedCommandHandlers(AfterProcessing: Boolean;
...
procedure TvxCustomSynEdit.DoOnClearBookmark(var Mark: TSynEditMark);
begin
  if Assigned(fOnClearMark) then
    fOnClearMark(Self, Mark);
end;

procedure TvxCustomSynEdit.DoOnPaintTransientEx(TransientType: TTransientType; Lock: Boolean);
var
  DoTransient: Boolean;
begin
...
procedure TvxCustomSynEdit.DoOnPaintTransient(TransientType: TTransientType);
begin
  DoOnPaintTransientEx(TransientType, False);
end;

procedure TvxCustomSynEdit.DoOnPaint;
begin
...
procedure TvxCustomSynEdit.DoOnPlaceMark(var Mark: TSynEditMark);
begin
  if Assigned(fOnPlaceMark) then
    fOnPlaceMark(Self, Mark);
end;

function TvxCustomSynEdit.DoOnReplaceText(const ASearch, AReplace: WideString;
...
procedure TvxCustomSynEdit.DoOnStatusChange(Changes: TSynStatusChanges);
begin
...
procedure TvxCustomSynEdit.UpdateModifiedStatus;
begin
  Modified := not UndoList.InitialState;
end;

procedure TvxCustomSynEdit.UndoRedoAdded(Sender: TObject);
begin
  UpdateModifiedStatus;
...
function TvxCustomSynEdit.GetWordAtRowCol(XY: TBufferCoord): WideString;
var
  Line: WideString;
  Len, Stop: Integer;
begin
...
function TvxCustomSynEdit.BufferToDisplayPos(const p: TBufferCoord): TDisplayCoord;
...
function TvxCustomSynEdit.DisplayToBufferPos(const p: TDisplayCoord): TBufferCoord;
...
procedure TvxCustomSynEdit.DoLinesDeleted(FirstLine, Count: Integer);
var
  i: Integer;
begin
  // gutter marks
...
procedure TvxCustomSynEdit.DoLinesInserted(FirstLine, Count: Integer);
var
  i: Integer;
begin
  // gutter marks
...
procedure TvxCustomSynEdit.PluginsAfterPaint(ACanvas: TvxCanvas; const AClip: TvxRect;
  FirstLine, LastLine: Integer);
var
  i: Integer;
begin
...
procedure TvxCustomSynEdit.ProperSetLine(ALine: Integer; const ALineText: WideString);
begin
  if eoTrimTrailingSpaces in Options then
...
procedure TvxCustomSynEdit.AddKeyUpHandler(aHandler: TKeyEvent);
begin
  fKbdHandler.AddKeyUpHandler(aHandler);
end;

procedure TvxCustomSynEdit.RemoveKeyUpHandler(aHandler: TKeyEvent);
begin
  fKbdHandler.RemoveKeyUpHandler(aHandler);
end;

procedure TvxCustomSynEdit.AddKeyDownHandler(aHandler: TKeyEvent);
begin
  fKbdHandler.AddKeyDownHandler(aHandler);
end;

procedure TvxCustomSynEdit.RemoveKeyDownHandler(aHandler: TKeyEvent);
begin
  fKbdHandler.RemoveKeyDownHandler(aHandler);
end;

procedure TvxCustomSynEdit.AddKeyPressHandler(aHandler: TKeyPressWEvent);
begin
  fKbdHandler.AddKeyPressHandler(aHandler);
end;

procedure TvxCustomSynEdit.RemoveKeyPressHandler(aHandler: TKeyPressWEvent);
begin
  fKbdHandler.RemoveKeyPressHandler(aHandler);
end;

procedure TvxCustomSynEdit.AddFocusControl(aControl: TWinControl);
begin
  fFocusList.Add(aControl);
end;

procedure TvxCustomSynEdit.RemoveFocusControl(aControl: TWinControl);
begin
  fFocusList.Remove(aControl);
end;

function TvxCustomSynEdit.IsIdentChar(AChar: WideChar): Boolean;
begin
...
function TvxCustomSynEdit.IsWhiteChar(AChar: WideChar): Boolean;
begin
  if Assigned(Highlighter) then
...
function TvxCustomSynEdit.IsWordBreakChar(AChar: WideChar): Boolean;
begin
  if Assigned(Highlighter) then
...
procedure TvxCustomSynEdit.SetSearchEngine(Value: TSynEditSearchCustom);
begin
...
function TvxCustomSynEdit.NextWordPos: TBufferCoord;
begin
  Result := NextWordPosEx(CaretXY);
end;

function TvxCustomSynEdit.WordStart: TBufferCoord;
begin
  Result := WordStartEx(CaretXY);
end;

function TvxCustomSynEdit.WordEnd: TBufferCoord;
begin
  Result := WordEndEx(CaretXY);
end;

function TvxCustomSynEdit.PrevWordPos: TBufferCoord;
begin
  Result := PrevWordPosEx(CaretXY);
end;

function TvxCustomSynEdit.GetPositionOfMouse(out aPos: TBufferCoord): Boolean;
...
function TvxCustomSynEdit.GetWordAtMouse: WideString;
var
  Point: TBufferCoord;
begin
...
function TvxCustomSynEdit.CharIndexToRowCol(Index: Integer): TBufferCoord;
...
function TvxCustomSynEdit.RowColToCharIndex(RowCol: TBufferCoord): Integer;
...
procedure TvxCustomSynEdit.Clear;
{ just to attain interface compatibility with TMemo }
begin
  ClearAll;
end;

function TvxCustomSynEdit.GetSelLength: Integer;
begin
  if SelAvail then
...
procedure TvxCustomSynEdit.SetSelLength(const Value: Integer);
var
...
procedure TvxCustomSynEdit.DefineProperties(Filer: TFiler);
  ...
  begin
    ...
    begin
      Result := not CollectionsEqual(Keystrokes,
        TvxCustomSynEdit(Filer.Ancestor).Keystrokes);
    end
    else begin
...
procedure TvxCustomSynEdit.DoChange;
begin
  if Assigned(fOnChange) then
    fOnChange(Self);
end;

procedure TvxCustomSynEdit.ReadAddedKeystrokes(Reader: TReader);
var
...
procedure TvxCustomSynEdit.ReadRemovedKeystrokes(Reader: TReader);
var
...
procedure TvxCustomSynEdit.WriteAddedKeystrokes(Writer: TWriter);
var
...
begin
  ...
  try
    if Writer.Ancestor <> nil then
      iDefaultKeys.Assign(TvxSynEdit(Writer.Ancestor).Keystrokes)
    else
...
procedure TvxCustomSynEdit.WriteRemovedKeystrokes(Writer: TWriter);
var
...
begin
  ...
  try
    if Writer.Ancestor <> nil then
      iRemovedKeys.Assign(TvxSynEdit(Writer.Ancestor).Keystrokes)
    else
...
procedure TvxCustomSynEdit.AddMouseDownHandler(aHandler: TMouseEvent);
begin
  fKbdHandler.AddMouseDownHandler(aHandler);
end;

procedure TvxCustomSynEdit.RemoveMouseDownHandler(aHandler: TMouseEvent);
begin
  fKbdHandler.RemoveMouseDownHandler(aHandler);
end;

procedure TvxCustomSynEdit.AddMouseUpHandler(aHandler: TMouseEvent);
begin
  fKbdHandler.AddMouseUpHandler(aHandler);
end;

procedure TvxCustomSynEdit.RemoveMouseUpHandler(aHandler: TMouseEvent);
begin
  fKbdHandler.RemoveMouseUpHandler(aHandler);
end;

procedure TvxCustomSynEdit.AddMouseCursorHandler(aHandler: TMouseCursorEvent);
begin
  fKbdHandler.AddMouseCursorHandler(aHandler);
end;

procedure TvxCustomSynEdit.RemoveMouseCursorHandler(aHandler: TMouseCursorEvent);
begin
  fKbdHandler.RemoveMouseCursorHandler(aHandler);
end;

procedure TvxCustomSynEdit.DoSearchFindFirstExecute(Action: TSearchFindFirst);
begin
...
procedure TvxCustomSynEdit.DoSearchFindExecute(Action: TSearchFind);
begin
  OnFindBeforeSearch := Action.Dialog.OnFind;
  OnCloseBeforeSearch := Action.Dialog.OnClose;
...
procedure TvxCustomSynEdit.DoSearchReplaceExecute(Action: TSearchReplace);
begin
  OnFindBeforeSearch := Action.Dialog.OnFind;
...
procedure TvxCustomSynEdit.DoSearchFindNextExecute(Action: TSearchFindNext);
begin
  SearchByFindDialog(Action.SearchFind.Dialog);
end;

procedure TvxCustomSynEdit.FindDialogFindFirst(Sender: TObject);
begin
...
procedure TvxCustomSynEdit.FindDialogFind(Sender: TObject);
begin
  if Sender is TFindDialog then
    SearchByFindDialog(TFindDialog(Sender));
end;

function TvxCustomSynEdit.SearchByFindDialog(FindDialog: TFindDialog) : bool;
var
...
procedure TvxCustomSynEdit.FindDialogClose(Sender: TObject);
begin
...
function TvxCustomSynEdit.GetWordWrap: Boolean;
begin
  Result := fWordWrapPlugin <> nil;
end;

procedure TvxCustomSynEdit.SetWordWrap(const Value: Boolean);
var
  vTempBlockBegin, vTempBlockEnd : TBufferCoord;
  vOldTopLine: Integer;
  vShowCaret: Boolean;
...
function TvxCustomSynEdit.GetDisplayLineCount: Integer;
begin
...
function TvxCustomSynEdit.LineToRow(aLine: Integer): Integer;
var
...
function TvxCustomSynEdit.RowToLine(aRow: Integer): Integer;
var
...
procedure TvxCustomSynEdit.SetInternalDisplayXY(const aPos: TDisplayCoord);
begin
  IncPaintLock;
...
procedure TvxCustomSynEdit.SetWantReturns(Value: Boolean);
begin
  fWantReturns := Value;
  {$IFDEF SYN_CLX}
...
procedure TvxCustomSynEdit.SetWantTabs(Value: Boolean);
begin
  fWantTabs := Value;
  {$IFDEF SYN_CLX}
  if fWantTabs then
...
procedure TvxCustomSynEdit.SetWordWrapGlyph(const Value: TSynGlyph);
begin
  fWordWrapGlyph.Assign(Value);
end;

procedure TvxCustomSynEdit.WordWrapGlyphChange(Sender: TObject);
begin
  if not (csLoading in ComponentState) then
    InvalidateGutter;
end;
...
initialization
  RegisterVGObjects('SynEdit', [TvxSynEdit]);
